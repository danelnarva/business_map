...
import { ICONOS_SERVICIOS, TRADUCCIONES_SERVICIOS } from "./servicios/serviciosConfig";

export default function ServiciosBarrios() {
  const { id } = useParams();
    ...
    fetch("/data/servicios_clasificados.geojson")
      ...
      .then(data => {
        const filtrados = data.features.filter(s => {
          const mismoBarrio = String(s.properties.id_barrio) === id;
          const tieneImagen = ICONOS_SERVICIOS[s.properties.amenity];
  ...
  const tipos = {};
  serviciosData?.features.forEach(s => {
    const tipo = s.properties.amenity;
        ...
        <div className="flex-1 bg-slate-800/80 backdrop-blur-sm border border-slate-700 rounded-2xl p-6 shadow-xl order-2 lg:order-1 flex flex-col justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-400">
              Total servicios en este barrio: <span className="text-emerald-400 font-bold">{serviciosData?.features.length || 0}</span>
            </p>
            ...
            <ul className="mt-4 text-sm grid grid-cols-1 sm:grid-cols-2 gap-3">
              {tiposVisibles.map(([tipo, count]) => (
                ...
                >
                  <span>{TRADUCCIONES_SERVICIOS[tipo] || tipo}</span>
